aws s3 mb s3://sm-source --region us-east-1
aws s3 mb s3://sm-destination --region us-west-1
echo "Buckets Created"

aws s3api put-bucket-versioning --bucket sm-source --versioning-configuration Status=Enabled
aws s3api put-bucket-versioning --bucket sm-destination --versioning-configuration Status=Enabled
...
s3 = boto3.client('s3')
REGION = 'us-east-1' #region where instance is launched.
...
aws lambda create-function --function-name pe-sm \
--runtime python3.6 \
...
--source-arn arn:aws:s3:::sm-source

echo "Getting ARN for the lambda function"
aws lambda get-function-configuration --function-name pe-sm \
